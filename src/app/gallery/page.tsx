        ...
        <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {PORTFOLIO_IMAGE_PATHS.map((imageSrc, index) => (
            <StaggerItem key={imageSrc}>
              <div className="relative aspect-[4/3] rounded-xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-500 group">
                <Image
                  src={imageSrc}
                  alt={
                    language === "es"
                      ? `Proyecto de pintura profesional de S&H Painting - Imagen ${index + 1}`
                      : `Professional painting project by S&H Painting - Showcase image ${index + 1}`
                  }
                  fill
